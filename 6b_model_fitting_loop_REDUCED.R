...
pdf(file=paste("figures/CEmodelGAMsmooths/Full-Reduce-modTest-REDUCED_Nov2017",runname,".pdf",sep=""),width=10,height=10)

options(warn=1) # print warnings as they occur
allwarnings = NULL
print(paste(length(allspp), 'models to fit'))
 
for(i in 1:length(allspp)){ #seq(from=41, to=705, by=20)){ 
  fittrain = TRUE
  ...
  if(length(levels(spdata$regionfact))==1){
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE))
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE))
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE))
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE))
  } else {
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
  }
  ...
  #save(mods, file=paste(modfolder, 'CEmods_',runname, '_', sp, '.RData', sep='')) 
  
  # write these files each time through the loop so that we can watch progress
  save(modeldiag, file=paste("output/modeldiag_Full-reduced-REDUCED_Nov2017",runname,".Rdata",sep=""))
