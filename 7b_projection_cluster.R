...
predsW <- matrix(data=NA, nrow=6187644, ncol=16)
  ...
  load(paste('GAM_models/CEmods_fitallreg_2017_', species, '.RData', sep=''))
  mygam1 <- mods[[1]]
  mygam2 <- mods[[2]]
  rm(mods)
  for(i in 1:2){
    ...
    for(j in 1:length(modelrun)){
      if(grepl('_Atl', species)){
        load(paste('Prediction_files/predictionEAST_rcp',rcp, '_', season, '_', modelrun[j], '.RData', sep=''))
        predictSpp <- pred.bathE
      } else{
        load(paste('Prediction_files/predictionWEST_rcp',rcp, '_', season, '_', modelrun[j], '.RData', sep=''))
    ...
    # Aggregate predictions into the 20year bins for plotting
